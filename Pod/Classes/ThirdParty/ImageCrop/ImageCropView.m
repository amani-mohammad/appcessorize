...
float IMAGE_MIN_HEIGHT = 300;//400;
float IMAGE_MIN_WIDTH = 300;//400;
...
- (void)viewDidLoad
{
    ...
    if (self) {
        ...
        contentView.backgroundColor = [UIColor colorWithRed:96.0/255.0 green:96.0/255.0 blue:96.0/255.0 alpha:0.9];
        contentView.frame = [[UIScreen mainScreen] bounds];
        
        self.title = @"appcessorize";
        
        self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc]
                                                 initWithImage:[UIImage imageNamed:[[Utility sharedInstance] appendBundleNameToString:COMMON_SCREEN_BACK_ICON]]
...
//        CGRect view = CGRectMake(0, 0, self.view.bounds.size.width, self.view.bounds.size.height - [[self navigationController] navigationBar].bounds.size.height - [[self navigationController] tabBarController].view.bounds.size.height);
//        NSLog(@"ImageCropView alloc] initWithFrame = %f, %f, %f, %f", view.origin.x, view.origin.y, view.size.width, view.size.height);
        
        CGRect view = IS_IPHONE? CGRectMake(0, 0, 320, 390):CGRectMake(0, 0, 768, 800);
...
- (void)addConfirmView {
    @try {
        NSString *confirmViewNibName;
        if (IS_IPHONE) {
            confirmViewNibName = @"BottomConfirmView";
        } else {
            confirmViewNibName = @"BottomConfirmView_iPad";
        }
        
        UIView *view = [[NSBundle mainBundle] loadNibNamed:[[Utility sharedInstance]appendBundleNameToString:confirmViewNibName] owner:self options:nil][0];
        view.frame = CGRectMake(0, self.view.bounds.size.height - (IS_IPHONE? 2.5:2.1)*view.bounds.size.height, self.view.bounds.size.width, view.bounds.size.height);
...
static CGFloat const DEFAULT_CONTROL_POINT_SIZE = 8;//5;
...
- (CGRect)clearAreaFromControlPoints {
    ...
    CGRect hole;
    if (isPortrait) {
        hole = CGRectMake(topLeftPoint.center.x - imageFrameInView.origin.x, topLeftPoint.center.y - imageFrameInView.origin.y, width, height);
    } else {
        hole = CGRectMake(topLeftPoint.center.x, topLeftPoint.center.y, width, height);
    }
    return hole;
}
...
//    imageView.frame = imageFrameInView;
    ...
    UIImage *blur;
    if (self.blurred) {
        blur = [image blurredImageWithRadius:15/*30*/ iterations:1 tintColor:[UIColor blackColor]];
    } else {
...
//    NSLog(@"cropAreaView frame %f %f", cropAreaView.frame.origin.x, cropAreaView.frame.origin.y);
//    
//    NSLog(@"clear frame %f %f", clearArea.origin.x, clearArea.origin.y);
